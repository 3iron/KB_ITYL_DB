...
select
    ...
    avg(menu_price)
from tbl_menu
group by category_code; -- category_code 값이 같은 행끼리 그루핑 처리(null도 하나의 그룹으로 처리)
...
group by
    category_code
having category_code is not null; -- 일반 컬럼 조건은 where 가 더 효율적
