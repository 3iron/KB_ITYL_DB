...
create database jhsdb;
# create schma jhsdb;

# 새로 생성한 계정에게 db 사용권한을 부여
grant all privileges on jhsdb.* to 'jhs'@'%';

# 권한 적용
flush privileges;

# 권한 적용이 안 돼서 아래 다시 실행 후 해결
# SELECT user, host FROM mysql.user WHERE user = 'jhs';
# CREATE DATABASE IF NOT EXISTS jdbc_ex;
# CREATE USER IF NOT EXISTS 'jhs'@'%' IDENTIFIED BY '비밀번호';
# GRANT ALL PRIVILEGES ON jdbc_ex.* TO 'jhs'@'%';
# FLUSH PRIVILEGES;
